...
from database.select import select_dict, execute_sql, insert, select_list
...
def get_schedule(provider):
    ...
    """
    _sql = provider.get('get_commission_schedule.sql')
...
def create_commissions(provider, teacher_ids: list, projects: list, defense_date: str):
    ...
    _sql_insert_o = provider.get('insert_commission_schedule.sql')     # INSERT commission_schedule (cs_date, project_id)
    _sql_insert_ol = provider.get('insert_commission_member.sql')   # INSERT commission_members (cs_id, teacher_id)
